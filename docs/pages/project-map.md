...
Key checks: use the map to select an affected relationship, inspect every primary source on that path, make the change, regenerate with `php cli/generate-project-map.php`, validate with `php cli/validate-project-map.php`, and then exercise the affected page/navigation behavior. A gap is a prompt to inspect existing source behavior before creating anything.
