...
- Keep public section gutters and repeated grids on the shared `1300px` content rail; update shared CSS before adding page-local inline padding fixes.
- Avoid unused asset files when replacing images or CSS.
- Keep client astrologer portraits in one circular, top-overlapping card frame across public surfaces. The supplied square portraits include unused template space above the face, so the visible frame must use a consistent face-focused crop instead of showing the raw square. Keep message, call, and profile icon controls aligned in one row. Varahi hero slides use transparent arch-shaped PNG cutouts with one visible image and ten accessible navigation dots; do not add an opaque image-card background.
