...
5. **Post-Merge Fork Sync:** After the upstream PR is merged, synchronize `bapxmediahub/bapXphpAiBackend` before reporting completion. Prefer the authenticated in-app browser at `https://github.com/bapxmediahub/bapXphpAiBackend/tree/main` and use **Sync fork → Update branch** when Browser control is available. If Browser control is unavailable, continue immediately with `gh repo sync bapxmediahub/bapXphpAiBackend --source getwinharris/bapXphpAiBackend --branch main`; this is the supported non-interactive fallback, not a blocker. Verify `gh api repos/getwinharris/bapXphpAiBackend/commits/main --jq .sha` equals `gh api repos/bapxmediahub/bapXphpAiBackend/commits/main --jq .sha`, and verify the fork's `workflow_dispatch` plus scheduled upstream-sync Action is enabled. If the fork contains divergent commits and cannot fast-forward, create an evidence-backed issue instead of forcing or discarding fork work.
