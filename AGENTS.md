...
5. **Post-Merge Fork Sync:** After the upstream PR is merged, open `https://github.com/bapxmediahub/bapXphpAiBackend/tree/main` in the authenticated in-app browser and use **Sync fork → Update branch**. Verify the fork `main` commit matches `getwinharris/bapXphpAiBackend` `main`, and verify the fork's manual/scheduled upstream-sync GitHub Action is enabled. Do not report the merge workflow complete while the fork is behind. If authenticated browser control is unavailable, report that blocker explicitly instead of claiming the sync was clicked.
