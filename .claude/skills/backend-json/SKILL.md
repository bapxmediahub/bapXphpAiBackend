...
php -l on every changed file, then `./bapXphp test`. For storage changes, exercise the affected admin page in a browser against the remote DB — `DatabaseService` swallows non-200 remote responses and returns `[]`, so a broken write shows up as silently empty data, never an error.
