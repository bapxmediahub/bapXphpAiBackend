...
Run the command with no arguments to confirm its usage output, then run it for real. Commands must stay non-interactive, credential-safe and shared-hosting compatible.

## Keeping this skill current

Update when a command is added or removed. Keep the CLI limited to map generation, schema listing, DB query and blog/product operations — the tooling coding agents rely on.
