        ...
        <div class="account-content">
            ...
            <div class="panel" style="margin-top:var(--space-lg);">
                ...
                <?php if(!empty($secrets['razorpay_key_id'])): ?>
                    ...
                    <script>
                    document.getElementById('wallet-pay-now').onclick=async()=>{const amount='<?= (int)$quote['amount_rupees'] ?>';const r=await fetch('/recharge/create-order',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({amount_rupees:amount})});const order=await r.json();if(order.error){showToast(order.error,'error');return;}new Razorpay({key:'<?= e($secrets['razorpay_key_id']) ?>',amount:order.amount,currency:'INR',order_id:order.id,name:'Sri Panchami Spiritual Credits',handler:async(resp)=>{const vr=await fetch('/recharge/verify',{method:'POST',body:new URLSearchParams({order_id:order.id,payment_id:resp.razorpay_payment_id,signature:resp.razorpay_signature,amount_rupees:amount})});const result=await vr.json();showToast(result.verified?'Recharge complete.':'Payment verification failed.',result.verified?'success':'error');if(result.verified)window.location.href='/account/wallet';}}).open();};
                    </script>
                <?php else: ?>
                    <script>document.addEventListener('DOMContentLoaded',function(){showToast('Razorpay is not configured yet. The recharge form is ready, but the payment gateway keys must be saved in Admin Integrations.','warning');});</script>
                <?php endif; ?>
            </div>
        </div>
    </div>
