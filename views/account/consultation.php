...
<section class="section consultation-room" data-session-id="<?= e($session['id'] ?? '') ?>" data-user-id="<?= e($currentUser['sub'] ?? '') ?>" data-role="<?= e($currentUser['role'] ?? 'customer') ?>" data-mode="<?= e($session['mode'] ?? '') ?>">
    ...
    <div class="consultation-room__grid">
        ...
        <aside class="panel consultation-call">
            ...
            <?php if($isAstrologer): ?><div class="session-controls"><button data-status="accepted" <?= in_array(($session['status'] ?? ''), ['queued','requested'], true) ? '' : 'disabled' ?>>Accept</button><button data-status="active" <?= ($session['status'] ?? '') === 'accepted' ? '' : 'disabled' ?>>Start Session</button><button data-status="completed" <?= ($session['status'] ?? '') === 'active' ? '' : 'disabled' ?>>Complete</button><button data-status="declined" <?= in_array(($session['status'] ?? ''), ['queued','requested'], true) ? '' : 'disabled' ?>>Decline</button></div><?php endif; ?>
