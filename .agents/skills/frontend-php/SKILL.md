...
- Keep public help guides in `content/docs/*.md` and route each guide through `/help/{slug}`. `/docs` is the index only because shared hosting exposes the physical repository `docs/` directory. Public guides explain customer tasks; internal operating notes remain separate.
- Treat offline consultation actions as truthful queued requests rather than dead controls. Do not label a provider online unless stored availability says so, and do not charge a queued request before acceptance.
