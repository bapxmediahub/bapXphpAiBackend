...
- When direct MySQL is unavailable, remote writes must use the authenticated, collection-allowlisted `DatabaseService` mutation protocol. Never expose arbitrary write SQL or secret records through the remote endpoint.
