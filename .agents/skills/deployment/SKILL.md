...
- Treat the root `.env` as a deployable repo file for shared-hosting auto-deploy (APP_NAME and APP_URL only). Admin credentials go through Admin → Settings. API secrets (Razorpay, SMTP, Google OAuth, support bot) go through Admin → Integrations and are stored in `settings.secrets.json` — never in `.env`. Keep generated runtime secret stores, lock files, logs, and backups ignored.
