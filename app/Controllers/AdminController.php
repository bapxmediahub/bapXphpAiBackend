...
final class AdminController extends BaseController {
    ...
    public function products(): void{$this->resource('Products','products',['name','description','price','offer_price','stock_status']);}
    ...
    public function astrologers(): void{$this->resource('Astrologers','astrologers',['name','description','email','price','modes','working_days','start_time','end_time','slot_minutes']);}
    ...
    private function save(string $collection): void{$data=array_filter($_POST,fn($v)=>$v!==''); if(isset($data['working_days']))$data['working_days']=array_map('trim',explode(',',$data['working_days'])); if(isset($data['modes']))$data['modes']=array_map('trim',explode(',',$data['modes'])); (new ResourceService($collection))->save($data); $this->flash('Saved.'); $this->redirect('/admin/'.$collection);}
