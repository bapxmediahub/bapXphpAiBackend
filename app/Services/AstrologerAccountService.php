...
final class AstrologerAccountService {
    ...
    public function sync(array $profile): array {
        ...
        foreach($users as $user){
            ...
            $user['username']=$username; $user['name']=$profile['name']??$user['name']??'';
            $this->store->upsert('users',$user);
            return $user;
        }
    ...
    public function deleteForSlug(string $slug): void {
        $users=$this->store->read('users');
        foreach($users as $user){
            if(($user['role']??'')==='astrologer'&&($user['astrologer_slug']??'')===$slug){
                $this->store->delete('users',$user['id']);
                return;
            }
        }
    }
